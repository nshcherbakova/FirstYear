...
QSwipeView::QSwipeView(QWidget *parent) : QStackedWidget(parent) {}
...
int QSwipeView::minSwipeDistance() const {
  ...
  return (m_swipeVertical ? frameRect().height() : frameRect().width()) / 3;
}
...
bool QSwipeView::gotoPrevPage() { return gotoPage(currentIndex() - 1); }

bool QSwipeView::gotoNextPage() { return gotoPage(currentIndex() + 1); }
