...
class QCompositeSwipeView : public QWidget {
  ...
  Q_PROPERTY(int currentIndex READ currentIndex WRITE setCurrentIndex NOTIFY
                 currentChanged STORED false);
  Q_PROPERTY(int count READ count STORED false);

  // This also acts like a QSwipeView:
  Q_PROPERTY(
      int animationSpeed READ animationSpeed WRITE animationSpeed STORED false);
  Q_PROPERTY(
      int swipeVelocity READ swipeVelocity WRITE swipeVelocity STORED false);
  Q_PROPERTY(
      bool swipeVertical READ swipeVertical WRITE swipeVertical STORED false);
